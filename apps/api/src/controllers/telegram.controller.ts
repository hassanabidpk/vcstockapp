...
export const telegramController = {
  async handleWebhook(req: Request, res: Response, _next: NextFunction) {
    ...
    // Respond to Telegram immediately, then run report in background
    // waitUntil keeps the serverless function alive after the response is sent
    waitUntil(
      reportService.generate(reportType).catch((err) => {
        logger.error({ err }, "Error processing Telegram webhook report");
      })
    );
